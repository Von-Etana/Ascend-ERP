...
class AdminPlansServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        register_admin_dashboard_item('admin-plans.snapshot', [
            ...
            'data' => fn () => [
                'metrics' => [
                    'total' => AdminPlan::query()->count(),
                    'active' => AdminPlan::query()->where('status', true)->count(),
                    'featured' => AdminPlan::query()->where('featured', true)->count(),
                    'free' => AdminPlan::query()->where('free_plan', true)->count(),
                ],
