...
class AdminPaymentSubscriptionsServiceProvider extends ServiceProvider
{
    ...
    public function boot(): void
    {
        ...
        register_admin_dashboard_item('admin-payment-subscriptions.snapshot', [
            ...
            'data' => fn () => [
                'metrics' => [
                    'total' => PaymentSubscription::query()->count(),
                    'active' => PaymentSubscription::query()->where('status', 1)->count(),
                    'cancelled' => PaymentSubscription::query()->where('status', 2)->count(),
                    'active_volume' => (float) PaymentSubscription::query()->where('status', 1)->sum('amount'),
                ],
