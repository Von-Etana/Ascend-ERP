...
>
    <x-ui.sub-header
        ...
        :description="$isEditing ? __('Refine identity, role permissions, and secure access for this internal account.') : __('Create an internal user with email, password, and the right role permissions.')"
    >
        <x-slot:actions>
        ...
        <x-ui.card padding="none">
            <div class="border-b px-5 py-4" style="border-color: var(--theme-border-color);">
                ...
                <h2 class="mt-2 text-[1.45rem] font-semibold tracking-[-0.03em]" style="color: var(--theme-header-text-color);">
                    {{ $isEditing ? __('Maintain account access') : __('Create a new internal user') }}
                </h2>
            </div>
            ...
            <form id="admin-user-form" wire:submit="save" class="space-y-7 p-5">
                ...
                <section class="space-y-4 border-t pt-7" style="border-color: var(--theme-border-color);">
                    <div>
                        ...
                        <h3 class="mt-2 text-base font-semibold" style="color: var(--theme-header-text-color);">{{ __('Role permissions and admin scope') }}</h3>
                    </div>
                    ...
                    <div class="grid gap-5 md:grid-cols-2" x-data="{ isSuperAdmin: $wire.entangle('form.is_super_admin').live }">
                        ...
                        <div x-show="!isSuperAdmin" x-cloak class="md:col-span-2">
                            ...
                            <p class="mt-2 text-sm" style="color: var(--theme-muted-text-color);">
                                {{ __('Roles define this user access to CRM, sales, finance, inventory, POS, marketing, inbox, automation, reporting, and administration modules.') }}
                                <a href="{{ route('admin-user-roles.index') }}" class="font-semibold text-[var(--theme-accent)] hover:underline" wire:navigate>{{ __('Manage roles and permissions') }}</a>
                            </p>
                        </div>
                    </div>
                </section>
                ...
                <x-ui.card>
                    ...
                    <div class="mt-4 grid gap-3">
                        @foreach ([
                            ...
                            ['label' => __('Access configured'), 'ready' => filled($form['role_id'] ?? null) || (bool) ($form['is_super_admin'] ?? false)],
                        ] as $item)
