@php
        ...
        $usersWithAccess = $userCollection->filter(fn ($user) => filled($user->role_id) || (bool) $user->is_super_admin)->count();
        $priorityUsers = $userCollection->filter(fn ($user) => blank($user->email) || (blank($user->role_id) && ! (bool) $user->is_super_admin))->count();
        ...
        $accessCoverage = $loadedUsers > 0 ? (int) round(($usersWithAccess / $loadedUsers) * 100) : 0;
        $authUser = auth()->user();
        ...
        $metricCards = [
            ...
            ['label' => __('Recent users'), 'value' => number_format($recentUsers), 'description' => __('Created in the last 7 days.'), 'tone' => '#10b981', 'progress' => $loadedUsers > 0 ? max(8, (int) round(($recentUsers / $loadedUsers) * 100)) : 8],
            ...
            ['label' => __('Access ready'), 'value' => $accessCoverage.'%', 'description' => number_format($usersWithAccess).' '.__('accounts with access'), 'tone' => '#64748b', 'progress' => max(8, $accessCoverage)],
        ];
...
<div class="mx-auto max-w-[88rem] space-y-6">
        ...
        <x-ui.page-hero
            ...
            :description="__('Create internal users, assign roles, grant permissions, and manage secure account access.')"
            :count="$totalUsers"
    ...
    >
        <x-ui.datatable-shell
                ...
                :info="__('Identity, role permissions, and contact posture for internal users.')"
                header-class="py-4"
            ...
            <div class="border-b px-6 py-5" style="border-color: var(--theme-border-color);">
                ...
                @forelse ($users as $user)
                        @php
                            $needsAttention = blank($user->email) || (blank($user->role_id) && ! (bool) $user->is_super_admin);
                        @endphp
                    <x-ui.card padding="md" wire:key="user-mobile-{{ $user->id }}">
                            ...
                            <div class="mt-4 grid gap-3 sm:grid-cols-2">
                                ...
                                <div class="rounded-[0.9rem] border px-3 py-3" style="border-color: var(--theme-border-color); background: var(--theme-surface-soft);">
                                    ...
                                    <p class="mt-2 text-sm font-medium" style="color: var(--theme-header-text-color);">{{ $user->is_super_admin ? __('Full access') : ($user->role?->name ?? __('Needs role')) }}</p>
                                    <p class="mt-1 text-xs" style="color: var(--theme-muted-text-color);">{{ $user->is_super_admin ? __('Super admin') : __('Role based access') }}</p>
                                </div>
                ...
                <x-ui.table class="rounded-none border-0 shadow-none">
                    <x-ui.table-head>
                        ...
                        <x-ui.table-body>
                            ...
                            @empty
                                <x-ui.table-row>
                                    <x-ui.table-cell :colspan="$internalWorkspace ? 6 : 7" class="py-10 text-center" style="color: var(--theme-muted-text-color);">{{ __('No users found.') }}</x-ui.table-cell>
                                </x-ui.table-row>
                            @endforelse
                        </x-ui.table-body>
                </x-ui.table>
