...
class UserForm extends Component
{
    ...
    public function save()
    {
        ...
        $internalWorkspace = app_internal_workspace_mode();

        $defaultSignupPlan = null;
        if (! $internalWorkspace && ! $user && ! filled($validated['form']['plan_id'])) {
        ...
        $payload = [
            ...
            'plan_id' => $internalWorkspace ? null : (filled($validated['form']['plan_id']) ? (int) $validated['form']['plan_id'] : $defaultSignupPlan?->id),
            'plan_expires_at' => $internalWorkspace ? null : ($validated['form']['plan_expires_at'] ?: null),
        ];
        ...
        log_activity('admin.users.create', 'Created a backend user.', [
            ...
            'metadata' => [
                ...
                'email' => $user->email,
                'team' => $team->name,
                'access' => $user->isSuperAdmin() ? 'super-admin' : ($user->role?->name ?: 'role-required'),
            ],
        ]);
    ...
    public function render(): View
    {
        ...
        return view('adminuser::livewire.user-form', [
            ...
            'plans' => app_internal_workspace_mode()
                ? new Collection
                : AdminPlan::query()->where('status', true)->orderBy('position')->orderBy('name')->get(['id', 'name']),
            'languages' => $languages,
