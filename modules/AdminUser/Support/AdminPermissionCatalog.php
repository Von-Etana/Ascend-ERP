...
class AdminPermissionCatalog
{
    ...
    public static function permissionForRoute(?string $routeName): ?string
    {
        ...
        if ($routeName === 'dashboard' || str_starts_with($routeName, 'dashboard.') || $routeName === 'portal.dashboard') {
            return null;
        }
        ...
        if (str_starts_with($routeName, 'admin-') || str_starts_with($routeName, 'settings.') || str_starts_with($routeName, 'portal.')) {
    ...
    protected static function catalogItems(): array
    {
        ...
        foreach (sidebar_registry()->editableSections('user') as $section) {
            foreach ($section['items'] ?? [] as $item) {
                array_push($items, ...self::flattenNavigationItem($item));
            }
        }
    ...
    {
        return __(Str::headline(str_replace(['admin-', 'settings.', 'portal.'], ['', '', ''], $group)));
    }
