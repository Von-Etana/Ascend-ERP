...
if (! function_exists('register_sidebar_item')) {
    ...
    {
        if (internal_workspace_navigation_blocked($item)) {
...
if (! function_exists('register_admin_dashboard_item')) {
    ...
    {
        if (app_internal_workspace_mode() && str_contains($id, 'marketplace')) {
            return admin_dashboard_registry();
        }
...
if (! function_exists('url_shortening_enabled')) {
    function url_shortening_enabled(?User $user = null): bool
    {
...
if (! function_exists('app_internal_workspace_mode')) {
    function app_internal_workspace_mode(): bool
    {
        return filter_var(env('APP_INTERNAL_WORKSPACE', true), FILTER_VALIDATE_BOOL);
    }
}

if (! function_exists('saas_features_enabled')) {
    function saas_features_enabled(): bool
    {
        return ! app_internal_workspace_mode();
    }
}

if (! function_exists('internal_workspace_blocked_routes')) {
    function internal_workspace_blocked_routes(): array
    {
        return [
            'admin-marketplace.',
            'admin-credits.',
...
if (! function_exists('internal_workspace_route_blocked')) {
    function internal_workspace_route_blocked(?string $routeName): bool
    {
        if (! app_internal_workspace_mode()) {
            return false;
        }
        ...
        foreach (internal_workspace_blocked_routes() as $blockedRoute) {
            if (str_ends_with($blockedRoute, '.') && str_starts_with($routeName, $blockedRoute)) {
                return true;
            }
...
if (! function_exists('internal_workspace_navigation_blocked')) {
    function internal_workspace_navigation_blocked(array $item): bool
    {
        if (! app_internal_workspace_mode()) {
            return false;
        }

        if (internal_workspace_route_blocked((string) ($item['route_name'] ?? ''))) {
            return true;
        }

        foreach ((array) ($item['active_when'] ?? []) as $pattern) {
            if (str_contains((string) $pattern, 'admin-marketplace')
                || str_contains((string) $pattern, 'admin-credits')
                || str_contains((string) $pattern, 'admin-plans')
                || str_contains((string) $pattern, 'admin-coupons')
                || str_contains((string) $pattern, 'admin-affiliate')
                || str_contains((string) $pattern, 'admin-support')
                || str_contains((string) $pattern, 'admin-payment')
                || str_contains((string) $pattern, 'portal.billing')
                || str_contains((string) $pattern, 'portal.invoices')
                || str_contains((string) $pattern, 'portal.packages')
                || str_contains((string) $pattern, 'portal.credits')
                || str_contains((string) $pattern, 'portal.support')
                || str_contains((string) $pattern, 'portal.affiliate')
                || str_contains((string) $pattern, 'guest.pricing')
            ) {
                return true;
            }
        }

        return str_contains((string) ($item['view'] ?? ''), 'marketplace');
    }
}
